...
Run the chunk below to load and clean the individual-level survey data. Note that men are not dropped here but a small number of respondents with missing data have been removed.
...
data <- read_dta("data/individual_characteristics.dta") %>%
    select(village, resp_gend, age, religion, caste, subcaste, educ, 
     ...
     mutate(gender = as.factor(ifelse(resp_gend == 1, "Male", "Female")),
           ...
           educ_none = ifelse(educ == 0, 1, 0), # No education
...
Q1. Use these data to estimate an LPM predicting whether a respondent worked in the previous week (`worked`) as a function of gender, caste, age, and education. Do not include any additional variables or transformations. Once you have the results, answer the question below.

***Note: The models covered in class used fixed-effects for villages to account for unmeasured village-level factors. Fixed-effects are not be required for the homework, but you can add them if you want (`+ village` is sufficient as the variable has already been cast as a factor). The tables will automatically suppress the fixed-effects from the regression output.***
...
modelsummary(list("LPM"=lpm), stars = TRUE, gof_omit = "AIC|BIC|RMSE",
             coef_omit = "village*", output = "huxtable")
```

Question: Write a paragraph describing the results of the model. Discuss each of the coefficients, including the intercept.
Answer:
...
modelsummary(list("LPM" = lpm, "Logistic" = logistic), stars = TRUE,
             gof_omit = "AIC|BIC|RMSE", coef_omit = "village*", output = "huxtable")
```
A: Take the coefficient for education and caste and explain the difference in interpretations between the LPM and the logistic regression model.
Answer:

B: The divide-by-4-rule helps to make the coefficients more comparable. Divide the logistic regression coefficients for gender and age by 4 and discuss the interpretation of each. Compare the results to those from the LPM.
Answer:
...
Q7: Modify the arguments of `modelsummary` to include an argument to exponentiate the coefficients in the results. Answer the question below.

```{r q4, echo=TRUE, tidy=TRUE}
modelsummary(list("Logistic" = logistic), stars = TRUE, gof_omit = "AIC|BIC|RMSE",
             coef_omit = "village*", output = "huxtable", ) # Add argument
```
Question: Write a paragraph describing the interpretation of the exponentiated coefficients for gender, age, and caste.
Answer: 

Q8. Estimate three more models predicting `worked` with alternative model specifications. You may use any combination of the following variables: `shg`, `gender`, `age`, `educ`, `educ_none`, `religion`, `caste`, `native`, `savings`. You are welcome to include transformations of these variables (e.g. logarithms, polynomials). Do not modify the dependent variable.
...
modelsummary(list(m1, m2, m3), stars = TRUE, gof_omit = "AIC|BIC|RMSE",
             coef_omit = "village*", output = "huxtable")
```

Q9. Use the log-likelihood scores in the table above to compare the three model. Discuss the differences between the three models and whether a particular model is preferable. You do not need to interpret every single coefficient, but you may want to make reference to any notable observations in your answer.
Answer:
...
Use the code chunk below to calculate the Efron's R-squared for each of the three models you estimated for Q8. Since you will need to do this three times, you might want to create a function.
...
modelsummary(list("Savings"=savings.logit),
             stars = TRUE, gof_omit = "AIC|BIC|RMSE", output = "huxtable")
```
...
Q12. When analyzing generalized linear models, it is often helpful to make predictions for average respondents. This can be more meaningful than interpreting coefficients. Construct a new dataset containing two rows, one for the average male and one for the average female. Use the mean value of each variable. The mean is undefined for caste, so set both values to "OBC" (or any group, as long as they are the same). Print this new dataset then use these average respondents to predict the probability that the average male and female respondent has a savings account. Answer the question below.
...
Estimate a Bayesian version of the model from Q13 using `stan_glm` and analyze the results. You can add fixed-effects if desired, but it will be considerably slower to estimate and you may need to adjust the number of iterations to ensure convergence. I have provided code to make a table showing the results of a fitted model.
```{r extra-credit, echo=TRUE, tidy=TRUE}
# Uncomment the relevant lines and complete the model code
# library(rstanarm)
# savings.bayes <- stan_glm() # Complete model code

# Note: It will also take a couple of minutes to produce this table
# modelsummary(list(savings.bayes), gof_omit = "AIC|BIC|RMSE", output = "huxtable", statistic = "conf.int")
```

Once you have estimated a model, use the chunk below to analyze the results. You can draw on any of the techniques we have covered in class so far. You may also estimate additional models if necessary for comparisons. Feel free to make more chunks as needed.
